...
10. Run `AxionPhoton_TIDMAD.ipynb` to produce the global Dark Matter Limit plot. This Jupyter notebook utilizes the plotting tools from AxionLimits (https://github.com/cajohare/AxionLimits) along with a specific plotting function `PlotFuncs_TIDMAD.py` for this project. Please note that AxionLimits requires input data in txt format. Therefore, convert `[output].csv` to `[output].txt` by removing all commas.
